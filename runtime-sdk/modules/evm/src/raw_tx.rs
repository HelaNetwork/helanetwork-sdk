...
lazy_static! {
    static ref KEY_CACHE: Mutex<LruCache<Vec<u8>, k256::ecdsa::VerifyingKey>> =
        Mutex::new(LruCache::new(NonZeroUsize::new(100000).unwrap()));
}
...
pub fn recover_low(
    ...
    if cached.is_some() {
        let verifying_key = cached.unwrap();
        return Ok(verifying_key);
    }

    // If not cached, recover and verify the key
    let verifying_key = 
