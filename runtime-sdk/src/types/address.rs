...
pub const ADDRESS_BECH32_HRP: &str = "hela0";
...
impl Address {
    ...
    pub fn eth_new(_ctx: &'static [u8], version: u8, data: &[u8]) -> Self {
        let mut a = [0; ADDRESS_SIZE];
        a[..ADDRESS_VERSION_SIZE].copy_from_slice(&[version]);
        a[ADDRESS_VERSION_SIZE..].copy_from_slice(data);
    ...
    pub fn into_eth_bytes(self) -> [u8; ADDRESS_DATA_SIZE] {
    ...
    pub fn eth_from_sigspec(spec: &SignatureAddressSpec) -> Self {
        match spec {
            SignatureAddressSpec::Ed25519(pk) => Address::new(
                ADDRESS_V0_ED25519_CONTEXT,
                ADDRESS_V0_VERSION,
            ...
            SignatureAddressSpec::Secp256k1Eth(pk) => Address::eth_new(
                ADDRESS_V0_SECP256K1ETH_CONTEXT,
                ADDRESS_V0_VERSION,
                // Use a scheme such that we can compute Secp256k1 addresses from Ethereum
                // addresses as this makes things more interoperable.
