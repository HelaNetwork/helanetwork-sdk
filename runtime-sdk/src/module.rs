    ...
    F: FnOnce(&mut C, B) -> Result<R, E>,
{
    DispatchResult::Handled((|| {
        // println!("gbtest: body in module.rs before decode is: {:?}", body);
        ...
        // println!("gbtest: body in module.rs args after decode is");
...
impl MethodHandler for Tuple {
    ...
    fn dispatch_query<C: Context>(
        ...
        // Return on first handler that can handle the method.
        // println!("gbtest query: dispatch_query in module.rs");
        for_tuples!( #(
            let args = match Tuple::dispatch_query::<C>(ctx, method, args.clone()) {
                DispatchResult::Handled(result) => {
                    // Print the result.
                    // println!("gbtest query: dispatch_query result in module.rs: {:?}", result);
